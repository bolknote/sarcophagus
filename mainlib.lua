...
function img_load (name)
	...
	--return love.graphics.newImage ("gr/"..name)
...
function game_autosave (name)
	...
	if m~='busy' then
		...
		savethread = love.thread.newThread('save.lua')
	...
	if debug then
		...
		for i,v in ipairs(files) do
			if v~=".DS_Store" then
				local f = {}
				f.name = v
				f.img = love.graphics.newImage ("gr/"..v)
		...
		local nl = 0
		local dimmax = 1024
		quad = love.graphics.newCanvas(dimmax,dimmax)
		...
		filedata = quad:newImageData( ):encode('png','quad.png')
		quad = love.graphics.newImage(quad:newImageData( ))
		images = nil
