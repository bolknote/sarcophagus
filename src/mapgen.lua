function chunk_load (name)
    --local save = love.filesystem.read ('assets/maps/'..name..'.map')
    local save = love.filesystem.read (name..'.map')
    
    if not save then
        save = love.filesystem.read ('assets/maps/'..name..'.map')
    end
...
local cmax = 1
local cave_pass_count = 5
for i=1,cave_pass_count do

    love.thread.getChannel('geninfo'):push(
        6 + (((i - 1) * 2) / (cave_pass_count * 2)) * 0.99
    )
    ...
    love.thread.getChannel('geninfo'):push(
        6 + ((((i - 1) * 2) + 1) / (cave_pass_count * 2)) * 0.99
    )
...
if true then
    ...
    for ci=1,cf.wmax*cf.wmax*2 do
        ...
        if ci%100 == 1 then
            ...
            local room_percent = math.floor(pc * 100)
            if room_percent > reported_room_percent then
                love.thread.getChannel('geninfo'):push(8 + pc)
                reported_room_percent = room_percent
            end
    ...
    local mn = 0
    local corner_report_interval = math.max(1, math.floor(cf.wmax / 100))

    --corners
    for ix=1,cf.wmax do

    if ix == 1 or ix % corner_report_interval == 0 then
        local corner_progress = math.min(0.99, (ix - 1) / cf.wmax)
        love.thread.getChannel('geninfo'):push(10 + corner_progress)
    end
